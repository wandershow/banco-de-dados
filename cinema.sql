...
--1: Lista dos filmes (Titulo, Ano, Realizador) franceses.
    select titulo, ano, realizador from filme where lower(pais) = 'france';

--2: Os anos em que o ator gabin estreou um filme.
     select distinct(filme.ano) from filme join distribuicao on filme.titulo = distribuicao.titulo where lower(ator) = 'gabin' ;
     
     select distinct(ano) from filme where titulo in (select titulo from distribuicao where lower(ator) = 'gabin' ;

--3: Os atores que trabalharam com o realizador truffaut.
    select distribuicao.ator from filme join distribuicao on filme.titulo = distribuicao.titulo where lower(filme.realizador) like 'truffaut';

--4: Lista dos filmes onde o realizador é também ator.
    select filme.titulo from filme join distribuicao on filme.titulo = distribuicao.titulo where lower(filme.realizador) = lower(distribuicao.ator);

    select titulo from((select ator, titulo from distribuicao) intersect (select realizador, titulo from filme)) as tmp1;   --**sempre que o select for no from tem que renomear
    
--5: O número de filmes onde eastwood trabalhou como ator.
    select count(filme.titulo) as filmes from filme join distribuicao on filme.titulo = distribuicao.titulo where lower(distribuicao.ator) like 'eastwood';
    
--6: O nome do cinema e o número da sala que apresenta um filme realizado por bovin. Dar também o nome do filme.
    select programa.nomcine, programa.sala, filme.titulo from programa join filme on programa.titulo = filme.titulo where lower(filme.realizador) = 'bovin' group by programa.nomcine, programa.sala, filme.titulo;
    
    select titulo, nomcine, sala from programa where titulo in (select titulo from filme where lower(realizador) = 'bovin') group by 1, 2, 3;
    
--7: O nome e o número de telefone dos cinemas que tem salas com capacidade para menos de 100 pessoas.
    select salacinema.nomcine, cinema.tel from salacinema join cinema on salacinema.nomcine = cinema.nomcine where salacinema.nulugares < 100;   
    
    select nomcine, tel from cinema where nomcine in (select nomcine from salacinema where nulugares < 100);

--8: Todos os atores que trabalharam com a atriz deneuve.
    select distinct(d2.ator) from distribuicao as d1 join distribuicao as d2 on d1.titulo = d2.titulo where d1.ator like 'deneuve' and d2.ator <> 'deneuve';
    
    select distinct(ator) from distribuicao where titulo in (select titulo from distribuicao where lower(ator) = 'deneuve') and lower(ator) <> 'deneuve';   

--9: As ruas e número de lugares dos cinemas onde passam os filmes de 1984.
        
        select cinema.rua, sum(salacinema.nulugares) as qtdlugares from cinema join salacinema on cinema.nomcine = salacinema.nomcine where cinema.nomcine in (select nomcine from programa where titulo in (select titulo from filme where ano = '1984')) group by cinema.rua;
            
    
--10: Realizadores que trabalharam como atores em filmes que eles mesmos não realizaram.    
    select ator,titulo from distribuicao where ator in (select realizador from filme)
    except
    select realizador, titulo from filme;	


--11: O número de atores de cada filme apresentado em qualquer sala de qualquer cinema.
    

    select programa.titulo, count(distinct(distribuicao.ator)) as qtd_ator from programa join distribuicao on programa.titulo = distribuicao.titulo where (programa.sala is not null) group by programa.titulo order by 2;
