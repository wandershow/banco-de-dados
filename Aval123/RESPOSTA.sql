...
SELECT * FROM venda;
 codigo |  vendedor   |   cliente   |    dia 
 
SELECT * FROM venda JOIN cliente ON venda.cliente = cliente.cpf;
 codigo |  vendedor   |   cliente   |    dia     |     cpf     |            nome             | nascimento
...
SELECT cliente.nome, SUM(tmp1.subtotal) AS gastos FROM (SELECT venda.codigo , SUM(item.quantidade*produto.preco) AS subtotal FROM venda JOIN item ON venda.codigo = item.venda JOIN produto ON item.produto = produto.codigo WHERE venda.dia BETWEEN '2000-02-01' AND '2000-02-29' GROUP BY venda.codigo;) AS tmp1 JOIN venda ON tmp1.codigo = venda.codigo JOIN cliente ON venda.cliente = cliente.cpf GROUP BY cliente.cpf ORDER BY 2 DESC;
