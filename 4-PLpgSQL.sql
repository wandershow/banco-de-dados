...
create or replace function km2mi (float) returns float as --km para milhas
	'select $1 * 0.6' -- '$1' significa primeiro parametro, '$2' significa o segundo parametro 
language 'sql';
select km2mi(20);
--\df lista as funçoes que eu criei
--\df+ lista todas as funçoes 


 -- função para transformar a data para o formato brasileiro
create or replace function ymd2dma_1 (timestamp) returns text as $$ -- usa '$$' para começar e terminar a function quando tiver aspas '' no meio dela.
...
insert into funcionario values (7, 'Funcionario 7', 1, 1500);

select max(salario) from funcionario where departamento = 1;

--Aqui pega apenas um numero...
x integer
x := count(*) from funcionario where departamento =1;
--aqui pega linha a linha
r record
...
declare
	r_funcionario record; --record e uma linha da tabela, grava uma linha da tabela
	salario real;
begin
	salario := 0;
	-- aqui pega linha a linha...
	for r_funcionario in select * from funcionario loop --faz um for linha a linha
		raise notice 'funcionario: %, %, %, %', r_funcionario.codigo, r_funcionario.nome, r_funcionario.departamento, r_funcionario.salario;--'raise notice' puxa um erro leve usado para debugar
...
create function calculasalariomedio() returns setof salariomedio as $$ -- 'returns setof' retorna varias colunas
declare
	r_departamento record;
	r_funcionario record;
	r_salariomedio salariomedio%rowtype; -- sempre vou usar o '%rowtype' quando usar um 'next'
	soma real;
...
begin
	for r_departamento in select * from departamento loop
		...
		return next r_salariomedio; --'next' = nao retorna ainda, acrescenta uma linha no resultado, nesse caso para cada departamento o codigo e a media de salario
	end loop;
	return;
end;
$$ language 'plpgsql';
select * from calculasalariomedio(); -- essa função retorna um setof

select funcionario.nome, funcionario.salario, departamento.codigo from funcionario join departamento on departamento.codigo = funcionario.departamento where funcionario.salario = (select max(salario) from funcionario where funcionario.departamento = departamento.codigo)  group by 1, 2, having funcionario.salario = (select max(funcionario.salario) from funcionario join departamento on departamento.codigo = funcionario.departamento where departamento.codigo = 1) order by 1 desc;

select *, (select max(salario) from funcionario where funcionario.departamento = departamento.codigo) from departamento;

select funcionario.nome, departamento.nome from funcionario join departamento on departamento.codigo = funcionario.departamento where funcionario.salario in (select (select max(salario) from funcionario where funcionario.departamento = departamento.codigo) from departamento) order by 2;
