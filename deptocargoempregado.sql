...
-- mostrar quantos cargos cada empregado teve
    select cargoempregado.empregado, count(cargoempregado.cargo) from cargoempregado join cargo on cargoempregado.cargo = cargo.codigo join empregado on cargoempregado.empregado = empregado.codigo group by 1 order by 2 desc;
    
 --betito solução
    select empregado.nome, count(*) as qtde
    from empregado
        join cargoempregado on empregadocodigo = cargoempregado.empregado
    group by empregado.codigo
    order by 2 desc;
    
-- mostrar o nome do empregado que teve mais cargos
   select empregado.nome, count(*) as qtde from empregado join cargoempregado on empregado.codigo = cargoempregado.empregado group by empregado.codigo having count(*) = (select count(*)
    from empregado
...
-- maneira com in
    ...
    from cargo
        join (select *
            from cargoempregado
            where (empregado, inicio) in (select cargoempregado.empregado, max(cargoempregado.inicio) as inicio 
            from cargoempregado
            group by 1)) as cargoatual on cargo.codigo = cargoatual.cargo
            join empregado on empregado.codigo = cargoatual.empregado;

-- outra   maneira
    select empregado.nome, cargo.descricao, cargo.salario
    from (select cargoempregado.*
        from cargoempregado
            join (select cargoempregado.empregado, max(cargoempregado.inicio) as inicio
        from cargoempregado
        group by 1) as tmp1 on cargoempregado.empregado = tmp1.empregado and cargoempregado.inicio = tmp1.inicio) as cargoatual
            join cargo on cargo.codigo = cargoatual.cargo
            join empregado on empregado.codigo = cargoatual.empregado;            
            
-- mostrar os empregados que atualmente trabalham no Depto 2    
    select deptoempregado.* 
    from (select deptoempregado.empregado, max(deptoempregado.inicio) as inicio
    from deptoempregado
    group by 1) as tmp1
    join deptoempregado on deptoempregado.empregado = tmp1.empregado and deptoempregado.inicio = tmp1.inicio
    where deptoempregado.depto = 2;
